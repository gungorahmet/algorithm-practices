...
class Water:
    ...
    def process_the_water(self, height):
        ...
        print(f"max_last_index: {max_last_index}")
        print("")

        i = 0
        while i < max_last_index:
            print(f"OUT ---> i:{i}, height[i]:{height[i]}")
            if height[i] > 0:
                start_to_collect = True

            if height[i] >= left_wall_val:
                left_wall_val = height[i]
                print(f"left_wall_val:{left_wall_val}")
                print("--------------------------------")
            ...
            if start_to_collect:
                for j in range(i + 1, max_last_index + 1):
                    print(f"IN ---> i:{i}, j:{j}, height[i]:{height[i]}, height[j]:{height[j]}")
                    if height[i] > height[j]:
                        ...
                        i = j
                        print(f"Else ---> i:{i}, j:{j}, left_wall_val:{left_wall_val}")
                        print("\n")
                        break
    ...
    def calculate_total_water(self):
        ...
        print(f"List till the latest max value: {self.height[0:max_last_index + 1]}")
        print(f"List reversed since the latest max value: {self.height[max_last_index::][::-1]}")
